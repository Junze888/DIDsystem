...
public class SdkBeanConfig {
    ...
    public Client client() throws Exception {
        ...
        String accountPath = this.config.getAccountPath();
        String accountFormat = this.config.getAccountFormat();
        ...
        for(String certPath: possibilities ) {
            try{
                ...
                configAccountPath(property, accountPath);//这里配置私钥没有装载

                ConfigOption configOption = new ConfigOption(property);
                configOption.getAccountConfig().setAccountFileFormat(accountFormat);
    ...
    public void configCryptoMaterial(ConfigProperty configProperty,String certPath) {
        ...
        Account.put("certPath", certPath);
        configProperty.setCryptoMaterial(Account);
    }

    public void configAccountPath(ConfigProperty configProperty,String AccountPath) {
        Map<String, Object> cryptoMaterials = new HashMap<>();
        cryptoMaterials.put("AccountPath", AccountPath);
        configProperty.setAccount(cryptoMaterials);
    }
